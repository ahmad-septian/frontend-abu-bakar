...
export default function EditJadwalPelajaranSd(props) {
  const {
    formDataJadwalPelajaran,
    handleChange,
    ClickCloseEditJadwalPelajaran,
    openEditJadwalPelajaran,
  } = props;
  return (
    <BootstrapDialog
      sx={{ backdropFilter: "blur(8px)" }}
      aria-labelledby="customized-dialog-title"
      open={openEditJadwalPelajaran}
      slots={{
    ...
    >
      <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
        Edit Jadwal Pelajaran SD
      </DialogTitle>
      <IconButton
        aria-label="close"
        onClick={ClickCloseEditJadwalPelajaran}
        sx={(theme) => ({
          position: "absolute",
          right: 8,
          top: 8,
